...
will return =False=. = 3 ~~ Int =, howver, will be =True=. 
...
with 3 + 4 actually being =adds(3,4)=.
...
that can act on other things, like this
#+BEGIN_EXAMPLE perl6
{ $_ * 2 }( 2 )
#+END_EXAMPLE
...
as you have seen, =$_= is a stand-in for whatever the function is
...
expression is true. What we are using is, in fact, this /chunk of
code/ or =Block=, and in fact, we can write it this way:
#+BEGIN_EXAMPLE perl6
(1..1000).grep( {$_ %% 11} )
#+END_EXAMPLE
...
totally equivalent and saves a character. This means that =Whatever=,
...
(1..1000).grep($_ %% 11)
#+END_EXAMPLE
will positively end in an error. /Pointy blocks/ to the
...
(1..1000).grep: -> $a {$a %% 11}
#+END_EXAMPLE
This is exactly the same as above, provided that whatever is behind
the arrow is alwo used as a variable there. This is more verbose for
this kind of expression, but it is even more verbose to white it this
way, which is also equivalent:
#+BEGIN_EXAMPLE perl6
(1..1000).grep: sub ($a) {$a %% 11}
#+END_EXAMPLE
We have followed a path that takes us all the way from simple code
...
using the oh so common foo and bar. In this case, the caret =^= iw
produced by typing wherever the caret is and then the space, since it
is a key that is usually composed as a circumflex accent with other characters to produce
things like /brûlé/, which looks like it has been made up, but is
actually French. 

 But do you remember that Perl 6
can easily deal with Unicode characters? From now on I propose to use
ß and þ as substitutes for the hackneyed c=foo= and =bar=, like here
#+BEGIN_EXAMPLE perl6
{ ([*] 1..$^ß ) / ( [*] 1..$^þ) * ([*] 1..($^ß - $^þ)) }(5,3)
#+END_EXAMPLE
