...
=~~= is kind of an universal comparator, but in case you are working
with strings, =>= is not going to work. Operators over strings are
...
#+BEGIN_EXAMPLE perl6
 332 ∈  (7,14...2000)
#+END_EXAMPLE
using =∈=, or the mathematical symbol for /belongs to/, which returns
False in this case. That can be an
easy and straightforward way of finding out multiples, for
instance. Multiples that [[https://trizen.gitbooks.io/perl6-rosettacode/content/programming_tasks/F/Filter.html][you can also extract using =grep=]]
#+BEGIN_EXAMPLE perl6
(1..100).grep(* %% 7)
#+END_EXAMPLE
where, in this case, =*= is a stand-in for the element, which we had
not used before. We had used it before as the end of a sequence
meaning /everything/. This is pretty much the same, /everyone/. Perl6
reuses symbols like this n different contexts, but at the same time it
gives them similar, or the same, meaning. That is good and well. 
...
school math class, using the same symbols. 
* ב‎ - Secure connection
    :PROPERTIES:
    :UNNUMBERED: t
    :END:
